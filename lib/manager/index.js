...
class Manager {
  ...
  source(sources) {
    for (const key in sources) {
      BoilerplateResolver.Sources.register( sources[key], key )
    }
  }
  ...
  ask(message, options) {
    ...
    options.skip = !!this.config('skip_ask')

    return Pending('ask').push({ message, options })
  }

  _ask(o) {
    const promise = ask(Print.ask.format(o.message), o.options)
