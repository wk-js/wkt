  ...
  methods: {
    ...
    t_ask_name() {
      return input(Print.ask.format('Project name: ')).then((name) => {
        this.parameters.name   = name.length > 0 ? name : 'noname'
        this.parameters.output = join( this.parameters.output, this.parameters.name )
        Print.debug(`Generate template to "${join(process.cwd(), this.parameters.output)}"`)
